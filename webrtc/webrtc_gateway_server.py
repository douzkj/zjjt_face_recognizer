...
from aiortc import VideoStreamTrack, RTCPeerConnection, RTCSessionDescription, RTCRtpCodecParameters
from aiohttp import web
from collections import defaultdict

from setup import UI_PROXY_PORT
...
async def offer(request):
    ...
    if not task or not task["active"]:
        return web.json_response({"error": "Invalid task"}, status=404)
...
# async def start_server(rtsp_url):
#     app = web.Application(
#         middlewares=[cors_middleware],
#         client_max_size=10 * 1024 * 1024  # 允许10MB大请求
#     )
#     app["frame_provider"] = FrameProvider(rtsp_url)
#
#     # 路由配置
...
#
#     # 静态文件服务
#     app.router.add_route("*", "/", lambda _: web.FileResponse("./client.html"))
#     app.router.add_static("/static/", path="./static")
#
#     # 关闭处理
#     async def on_shutdown(app):
#         print("关闭服务器...")
#         app["frame_provider"].stop()
...
# ====================== 接口处理 ======================
async def handle_start_pull(request):
    """启动转码任务接口"""
    ...
    if not stream_url:
        return web.json_response({"error": "Missing stream_url"}, status=400)

    try:
        # 创建转码任务
        task_id = transcoder.create_task(stream_url)
        pull_url = f"http://{request.host}:{UI_PROXY_PORT}/offer/{task_id}"
...
async def handle_stop_pull(request):
    """停止转码任务接口"""
    data = await request.json()
...
def create_app():
    ...
    # 注册接口
    app.router.add_post('/api/stream-manage-gw/start-pull-trans', handle_start_pull)
    app.router.add_post('/api/stream-manage-gw/stop-pull-trans', handle_stop_pull)
    app.router.add_post('/offer/{task_id}', offer)

    # 静态文件和健康检查
    app.router.add_get('/api/health', lambda _: web.json_response({"status": "ok"}))

    # 关闭处理
    async def on_shutdown():
        print("关闭服务器...")
        transcoder.stop_all_task()
        await asyncio.sleep(1)  # 等待资源释放
